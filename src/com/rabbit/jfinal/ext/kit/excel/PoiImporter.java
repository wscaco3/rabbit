...
import com.rabbit.jfinal.ext.kit.Reflect;
import com.rabbit.jfinal.ext.kit.excel.convert.CellConvert;
import com.rabbit.jfinal.ext.kit.excel.filter.RowFilter;
import com.rabbit.jfinal.ext.kit.excel.validate.CellValidate;
...
//                            format.setGroupingUsed(false);
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (valid) {
                ...
                if (StrKit.notBlank(convertClassName)) {
                    CellConvert<Model> cellConvert = Reflect.on(convertClassName).get();
    ...
    public static List<Record> recordSheet(File file, Rule rule) {
        int start = rule.getStart();
        int end = rule.getEnd();
        Workbook wb;
        try {
        ...
        for (int rowIndex = start; rowIndex <= end; rowIndex++) {
            ...
            for (int cellIndex = row.getFirstCellNum(); cellIndex < cellNum; cellIndex++) {
                ...
                Rule.Cell rcell = matchCell(rule,cellIndex);
                if(rcell!=null){
                    columns.set(rcell.getAttribute(),column.trim());
                }
            }
            result.add(columns);
        }
